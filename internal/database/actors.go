...
func (db *DB) CountPeers(ctx context.Context) (int, error) {
	var count int
	if err := db.bun.NewRaw("SELECT COUNT(*) FROM actors").Scan(ctx, &count); err != nil {
...
func (db *DB) RemoveFollow(ctx context.Context, actorURL string) error {
	res, err := db.bun.NewRaw(
		"UPDATE actors SET they_follow_us = FALSE, they_follow_us_at = NULL WHERE actor_url = ? AND they_follow_us = TRUE",
		actorURL).Exec(ctx)
	if err != nil {
...
func (db *DB) CountFollows(ctx context.Context) (int, error) {
	var count int
	if err := db.bun.NewRaw("SELECT COUNT(*) FROM actors WHERE they_follow_us = TRUE").Scan(ctx, &count); err != nil {
...
func (db *DB) RefreshFollow(ctx context.Context, actorURL, publicKeyPEM, endpoint string, alias *string) error {
	res, err := db.bun.NewRaw(
		"UPDATE actors SET public_key_pem = ?, endpoint = ?, alias = ? WHERE actor_url = ? AND they_follow_us = TRUE",
		publicKeyPEM, endpoint, alias, actorURL).Exec(ctx)
	if err != nil {
...
func (db *DB) AcceptOutgoingFollow(ctx context.Context, actorURL string) error {
	res, err := db.bun.NewRaw(`
		UPDATE actors
		SET we_follow_status = 'accepted', we_follow_accept_at = CURRENT_TIMESTAMP
		WHERE actor_url = ? AND we_follow_them = TRUE AND we_follow_status = 'pending'`,
		actorURL).Exec(ctx)
	if err != nil {
...
func (db *DB) RejectOutgoingFollow(ctx context.Context, actorURL string) error {
	_, err := db.bun.NewRaw(`
		UPDATE actors SET we_follow_status = 'rejected'
		WHERE actor_url = ? AND we_follow_them = TRUE AND we_follow_status = 'pending'`,
		actorURL).Exec(ctx)
	if err != nil {
...
func (db *DB) RemoveOutgoingFollow(ctx context.Context, actorURL string) error {
	res, err := db.bun.NewRaw(
		"UPDATE actors SET we_follow_them = FALSE, we_follow_status = NULL, we_follow_accept_at = NULL WHERE actor_url = ? AND we_follow_them = TRUE",
		actorURL).Exec(ctx)
	if err != nil {
...
func (db *DB) CountOutgoingFollows(ctx context.Context, status string) (int, error) {
	var n int
	err := db.bun.NewRaw(
		"SELECT COUNT(*) FROM actors WHERE we_follow_them = TRUE AND we_follow_status = ?", status).Scan(ctx, &n)
	if err != nil {
...
func (db *DB) SetActorHealth(ctx context.Context, actorURL string, healthy bool) error {
	_, err := db.bun.NewRaw(
		"UPDATE actors SET is_healthy = ?, last_seen_at = CURRENT_TIMESTAMP WHERE actor_url = ?",
